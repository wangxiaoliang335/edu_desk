...
static QPixmap loadTrimmedScaledPixmap(const QString& path, int targetSize, int yOffset = 0)
{
    ...
    // 裁掉四周的透明留白（很多 @2x/@3x 图会有不对称留白，导致视觉重心偏下）
    int left = img.width();
    int right = -1;
    int top = img.height();
    int bottom = -1;
    ...
    m_visibleCloseButton(true)
{
    ...
    // 手机号输入
    const int kInputIconSize = 40; // 图标显示尺寸
    const int kInputIconYOffset = 5; // 图形本体微调：负数上移，正数下移，0 不偏移
    ...
    pwdIcon->setPixmap(loadTrimmedScaledPixmap(":/res/img/signin_ic_password@3x.png", kInputIconSize, kInputIconYOffset));
    pwdIcon->setFixedSize(kInputIconSize, kInputIconSize);
    ...
    pwdLayout->addWidget(pwdIcon, 0, Qt::AlignVCenter);
    ...
    secPwdIcon->setPixmap(loadTrimmedScaledPixmap(":/res/img/signin_ic_password@3x.png", kInputIconSize, kInputIconYOffset));
    secPwdIcon->setFixedSize(kInputIconSize, kInputIconSize);
    ...
    secPwdLayout->addWidget(secPwdIcon, 0, Qt::AlignVCenter);
...
void RegisterDialog::setTitleName(const QString& name) {
    m_titleName = name;
    update();
}

void RegisterDialog::visibleCloseButton(bool val)
{
    m_visibleCloseButton = val;
}

void RegisterDialog::setBackgroundColor(const QColor& color) {
    m_backgroundColor = color;
    update();
}

void RegisterDialog::setBorderColor(const QColor& color) {
    m_borderColor = color;
    update();
}

void RegisterDialog::setBorderWidth(int val) {
    m_borderWidth = val;
    update();
}

void RegisterDialog::setRadius(int val) {
    m_radius = val;
    update();
}
...
void RegisterDialog::leaveEvent(QEvent* event)
{
    QDialog::leaveEvent(event);
    closeButton->hide();
}

void RegisterDialog::enterEvent(QEvent* event)
{
...
void RegisterDialog::onTimerTick()
{
    countdownValue--;
    if (countdownValue <= 0) {
        countdownTimer.stop();
...
void RegisterDialog::onLoginClicked()
{
